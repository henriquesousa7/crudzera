...
def set_data():
	if request.method == 'POST':
		...
		return redirect(url_for('sending', name=name, age=age, gender=gender))
	else:

		return render_template('form.html')

@app.route('/sending/<name>&<age>&<gender>', methods=['POST'])
def sending(name, age, gender):
	values = {'name':name, 'age':age, 'gender':gender}

	post_id = db.people.insert_one(values).inserted_id
